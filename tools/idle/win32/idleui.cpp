...
//
DLLEXPORT BOOL IdleUIInit() { return TRUE; }
...
//
DLLEXPORT DWORD IdleUIGetLastInputTime() { return g_lastInputTick; }
...
LRESULT CALLBACK mouseHookCallback(int code, WPARAM wParam, LPARAM lParam)
{
    if (code == HC_ACTION) {
        ...
        if (wParam == WM_MOUSEMOVE && lParam != 0) {
            ...
            if (mhs->pt.x != g_lastMousePos.x || mhs->pt.y != g_lastMousePos.y) {
                change         = true;
